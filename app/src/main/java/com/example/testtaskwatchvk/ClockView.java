package com.example.testtaskwatchvk;
...
public class ClockView  extends View{
    ...
    private void drawHandHour(Canvas canvas, double loc) {
    ...
    private void drawHandMinute(Canvas canvas, double loc) {
        double angle = Math.PI * loc / 30 -Math.PI / 2;
        int handRadius = radius - handTruncation;
        paint.setStrokeWidth(15);
        canvas.drawLine(width / 2, height / 2,(float)(width / 2 + Math.cos(angle) * handRadius), (float)(height / 2 + Math.sin(angle) * handRadius), paint);
    }
    private void drawHandsecond(Canvas canvas, double loc) {
        double angle = Math.PI * loc / 30 -Math.PI / 2;
        int handRadius = radius - handTruncation;
    ...
    private void drawHands(Canvas canvas) {
        ...
        drawHandHour(canvas,(hour + c.get(Calendar.MINUTE) / 60) * 5f);
        drawHandMinute(canvas,c.get(Calendar.MINUTE));
        drawHandsecond(canvas,c.get(Calendar.SECOND));
